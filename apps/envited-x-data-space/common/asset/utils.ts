...
export const extractDomainMetadata = (jsonData: Record<string, any>) => {
  ...
  const dataResource = Object.entries(jsonData).find(([key]) => key.endsWith(':hasDataResource'))?.[1] as Record<string, any>
  const dataResourceExtension = Object.entries(jsonData).find(([key]) => key.endsWith(':hasDataResourceExtension'))?.[1] as Record<string, any>
