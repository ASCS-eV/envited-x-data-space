...
import { has, isNotNil, pathOr, toPairs } from 'ramda'
...
export const AssetDynamic: FC<AssetProps> = ({ sections, metadata }) => {
  const token = {
    name: '',
    displayUri: '',
    tokenAttributes: '',
  ...
  useEffect(() => {
    ...
    setDisplayTree(displayTrees.hdmapNew)
  }, [attributes])
                ...
                {displayTree?.categories.map((tab: { name: string; sections: string[] }) => (
                  <TabPanel key={tab.name}>
                    ...
                    {tab.sections.map(section => (
                      <>
                        <h3 className="text-lg font-medium mt-6">{sections[section].name}</h3>
                        <div className="mt-0">
                          <dl className="grid grid-cols-1 sm:grid-cols-2 pt-2">
                            {isNotNil(sections[section].properties) &&
                              toPairs(sections[section].properties).map(([key, item]) => (
                                <div
                                  key={item.name}
