...
describe('common/serverActions/users/activateUserById', () => {
  it('should return a activated user as expected', async () => {
    ...
    const result = await SUT._activateUserById({ db: dbStub, getServerSession: getServerSessionStub, log: logStub })(
      'USER_ID',
    )
  ...
  it('should throw because of missing session', async () => {
    ...
    await expect(
      SUT._activateUserById({ db: dbStub, getServerSession: getServerSessionStub, log: logStub })('USER_ID'),
