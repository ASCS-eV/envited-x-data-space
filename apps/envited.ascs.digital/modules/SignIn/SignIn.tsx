...
export const SignIn: FC<{ colorScheme?: ColorScheme }> = ({ colorScheme = ColorScheme.dark }) => {
  ...
  return !isNil(session) ? (
    ...
    <button
      onClick={() => signIn({ pkh: 'tz1PRINCIPAL' })}
      className={`${
    ...
    >
      <span
        className={`absolute right-0 w-8 h-32 -mt-12 transition-all duration-1000 transform translate-x-12 bg-white opacity-10 rotate-12 ease duration-1000 group-hover:-translate-x-40`}
      ></span>
